...
const MOCK_GIFT_PRODUCTS = [
  ...
  { id: 2, name: "Coffret Invités Premium - 5 personnes", price: 99.0, compareAtPrice: null, image: "/assets/logo.png" },
  { id: 3, name: "Set Cadeau Henné + Huile Soin", price: 39.9, compareAtPrice: 45.0, image: "/assets/logo.png" },
  { id: 4, name: "Coffret Sur-Mesure Mariage", price: 149.0, compareAtPrice: null, image: "/assets/logo.png" },
] as const;
  ...
  return (
    <main>
      ...
      <section className="section-padding bg-white">
        <div className="container-narrow">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="font-heading mb-6 text-2xl font-bold text-text md:text-3xl">Le concept</h2>
            <p className="text-text-light leading-relaxed">Pour sublimer votre mariage ou événement, offrez à vos invités des cadeaux henné qui marqueront les esprits. Nos coffrets personnalisables incluent cônes de henné de qualité, accessoires et soins post-application. Une attention délicate qui transforme chaque invité en participant actif de votre célébration.</p>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding bg-warm/30">
        <div className="container-narrow">
          <h2 className="font-heading mb-10 text-center text-2xl font-bold text-text md:text-3xl">Nos coffrets cadeaux</h2>
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {MOCK_GIFT_PRODUCTS.map((product) => (
              <article key={product.id} className="group flex flex-col overflow-hidden rounded-2xl border border-warm-dark/20 bg-white shadow-sm transition-shadow hover:shadow-lg">
                <div className="relative aspect-square overflow-hidden">
                  <Image src={product.image} alt={product.name} fill className="object-cover transition-transform duration-300 group-hover:scale-105" sizes="(max-width: 640px) 100vw, 25vw" />
                </div>
